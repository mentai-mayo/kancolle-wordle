...
window.addEventListener('load', ()=>{
  ...
  const ShipList = [
    ...
    { id: 147, chars: 'ヴェールヌイ'  , name: 'Верный'                , type: '駆逐艦'      , class: '暁型'                                 },
    ...
    { id: 589, chars: 'アブルッツィ'  , name: 'L. d. S. D. d. Abrizzi', type: '軽巡洋艦'    , class: 'Luigi di Savoia Duca degli Abrizzi級' },
    { id: 590, chars: 'ガリバルディ'  , name: 'G. Garibaldi'          , type: '軽巡洋艦'    , class: 'Luigi di Savoia Duca degli Abrizzi級' },
  ...
  function cmdlineInput(command){
    if(command[0] == '/'){
      ...
      if(/^([\u30a1-\u30f4]|[\u3041-\u3094]|\u30fc)+$/.test(command) && !/\u3090|\u3091/.test(command)){
        const input = convertHiragana(command);
